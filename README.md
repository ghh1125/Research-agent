...
第 2 节详细说明 Streamlit，第 3 节详细说明独立拖拽工作流，第 4 节说明 CLI；第 5 节以后介绍节点、代码和报告结构。
...
workflow_server.py               # 独立拖拽工作流 HTTP 服务入口
workflow_web/
  index.html                     # 拖拽工作流页面结构
  app.js                         # 画布编辑、节点配置、运行轮询和人工审核
  styles.css                     # 拖拽页面样式
examples/
...
src/
  ...
  visual_workflow/
    api_server.py                # 拖拽页面 HTTP API、上传和运行接口
    graph.py                     # 工作流校验与稳定拓扑排序
    executor.py                  # 根据画布快照逐节点执行
    registry.py                  # 节点类型到 Pipeline/业务函数的映射
    run_store.py                 # 后台运行状态和人工检查点
  nodes/
    start.py
    project_overview.py
...
tests/
  ...
  test_visual_workflow_*.py      # 独立拖拽工作流后端测试
  test_workflow_web.py           # 拖拽页面结构与功能入口测试
  test_cli.py
```
