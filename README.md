...
除 Streamlit 分步向导外，项目还提供一个独立 Web 页面，用于调整节点后一次性执行工作流：
...
浏览器打开 `http://127.0.0.1:8765`。初始画布就是当前完整流程，可以拖拽新增节点、移动、连线和删除节点。点击节点可配置该节点字段并上传对应文件；例如“开始”节点上传 BP，“深度尽调”节点上传五类材料，五个单项尽调节点分别上传自己的材料。

运行前必须完成节点、连线、文件、模型和 Prompt 配置。点击“运行工作流”后，画布和节点配置会锁定，系统使用启动时的工作流快照按 DAG 拓扑顺序执行；项目基本概况、行业深度分析、竞品选择和竞品矩阵报告审核会暂停等待人工操作。节点缺少所需上游输出时会标记为 `skipped`，不会阻止没有依赖关系的其他节点执行。

默认“深度尽调”节点仍运行五个专项模块并生成汇总。也可以删除该节点，拖入团队、业务、财务、技术与知识产权、法律五个单项节点；当五份报告齐全时，估值节点会自动组装深度尽调汇总后继续。API Key 只保存在本次后台运行的内存中，不进入工作流 JSON、运行事件或报告。

#### 节点 LLM 配置

全局默认百炼模型为 `qwen3.7-plus`。可选模型包括：
...
第三方模型需要先在阿里云百炼控制台开通对应服务，并使用华北 2（北京）地域的 API Key。点击画布节点后，可以只为当前节点实例修改：

- 节点名称和描述；
- 每个内部 LLM 步骤的模型；
- 每个内部 LLM 步骤的完整 Prompt。

竞品矩阵分析分别提供“单家竞品分析”和“竞品统一汇总”；深度尽调分别提供团队、业务、财务、技术/IP、法律五个 Prompt。Prompt 下方会逐项说明变量代表的材料、材料来源和写法。例如 `{bp_text}` 表示开始节点上传 BP 后解析出的全文，Prompt 中保留 `{bp_text}`，运行时会自动替换为实际材料。写入未知变量会在工作流校验时被拒绝；系统消息、JSON Schema 和 JSON 修复约束不可修改。
...
| BP 文件 | 留空 |
| 五类尽调材料 | 首次测试全部留空；程序应在报告中标注资料不足 |

为了减少真实 API 成本，竞品发现后只勾选 1-2 家最相关竞品。

#### 手工测试场景 A：默认完整流程

1. 点击“恢复默认”。
2. 点击“开始”节点，填入“共用测试输入”。
3. 点击“竞品矩阵分析”节点，展开两个 LLM 步骤：
   - 单家竞品分析：选择 `qwen3.6-flash`；
   - 竞品统一汇总：保留 `qwen3.7-plus`。
4. 在“竞品统一汇总”默认 Prompt 末尾追加：`矩阵结论必须区分已核实事实、合理推断和信息缺口。`
5. 点击“校验”，应提示通过。
6. 点击“运行工作流”，依次审核项目概况、行业分析、竞品选择和竞品报告。
7. 预期所有 8 个默认节点 `completed`，最终报告包含五项尽调和估值。

#### 手工测试场景 B：仅团队尽调

1. 点击“恢复默认”。
2. 删除“深度尽调”节点；删除操作会同时删除它关联的两条边。
3. 从左侧点击“团队尽调”新增节点。
4. 连线：
   - `竞品矩阵分析 → 团队尽调`
   - `团队尽调 → 估值分析`
5. 点击团队节点，将名称改为“核心团队专项核验”，描述改为“本次仅执行团队维度尽调”。
6. 模型选择 `qwen3.6-flash`，在默认 Prompt 末尾追加：`重点检查创始团队履历连续性、关键人依赖和股权稳定性。`
7. 填入“共用测试输入”并运行。
8. 预期团队尽调、估值分析、最终报告均为 `completed`，不能出现 `skipped`。
9. 估值和最终报告应明确写出：未执行业务、财务、技术与知识产权、法律尽调，结论置信度较低。
...
3. 连线：
   ...
   - `团队尽调 → 估值分析`
   - `财务尽调 → 估值分析`
4. 两个节点在画布上可以上下并列；执行器仍会逐个执行，不会同时请求百炼。
5. 团队模型选择 `qwen3.6-flash`，财务模型选择 `qwen3.7-plus`。
6. 填入“共用测试输入”并运行。
7. 预期估值和最终报告 `completed`，报告标注已执行团队、财务尽调，未执行业务、技术/IP、法律尽调。

#### 手工测试场景 D：五个专项尽调分支

1. 点击“恢复默认”，删除“深度尽调”。
2. 新增团队、业务、财务、技术与知识产权、法律五个专项节点。
3. 从“竞品矩阵分析”分别连接到五个专项节点。
4. 从五个专项节点分别连接到“估值分析”。
5. 五个节点可在画布上并列排列，但实际按稳定拓扑顺序串行执行。
6. 可为每个节点选择不同模型；建议团队/财务/法律使用 `qwen3.7-plus`，业务/技术使用 `qwen3.6-flash`。
7. 填入“共用测试输入”并运行。
8. 预期五个专项、估值和最终报告全部 `completed`；深度尽调汇总显示五类均已执行，不存在“未执行专项尽调”。
