...
2. 给每个竞品分配独立的证据文本预算，避免前面竞品的长资料挤掉后面的竞品。
3. 把目标公司、行业结论和全部已选竞品放进**同一次 LLM 分析调用**。
4. 使用统一口径横向比较产品能力、商业模式、客户与场景、技术壁垒、商业化与融资进展。
5. 最终生成**一份统一报告**，包含逐家竞品画像、能力矩阵、SWOT 和目标公司定位判断。

因此当前实现不是"每个竞品单独调用一个 LLM、各生成一份报告"，也不是多个 LLM 并行分析；它是"逐家收集证据，再统一综合分析"，这样横向矩阵的评价口径更一致。

### 2.5 查看竞品矩阵分析报告

报告生成后，页面只读展示完整 Markdown，并提供 Markdown/DOCX 下载。这里不显示尽调材料上传框，也不提供反馈重新生成。

确认报告后点击 **"进入深度尽调"**，才会进入下一步。
