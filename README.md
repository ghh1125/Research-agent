...
| 入口 | 启动文件 | 页面/交互 | 业务编排 |
| --- | --- | --- | --- |
| Streamlit 分步向导 | `app.py` | Streamlit 多阶段页面 | `BPPipeline` |
| 独立拖拽工作流 | `workflow_server.py` + `workflow_web/` | 原生 Web 画布 + HTTP API | `WorkflowExecutor` → `BPPipeline` / 节点函数 |
| CLI | `main.py` | 终端参数与人工输入 | `BPPipeline` |
...
```text
research-agent/
├── app.py                         # Streamlit 固定流程入口
├── workflow_server.py             # 独立拖拽工作流服务入口
├── main.py                        # CLI 入口
│
├── workflow_web/                  # 独立拖拽页面
│   ├── index.html                 # 页面结构
│   ├── app.js                     # 画布、节点配置、运行和人工审核
│   └── styles.css                 # 页面样式
│
├── examples/
│   └── research_workflow.json     # 默认完整画布
│
├── src/
│   ├── pipeline.py                # 固定 Pipeline 编排和节点调用封装
│   ├── schema.py                  # 节点输入输出 Pydantic 模型
│   ├── llm.py                     # 百炼/OpenAI 兼容模型调用与 JSON 校验
│   ├── search.py                  # Serper/Tavily/Google 搜索
│   ├── files.py                   # PDF/PPT/Word/Excel 文件解析
│   ├── report.py                  # Markdown 和 DOCX 报告输出
│   │
│   ├── visual_workflow/           # 拖拽工作流运行层
│   │   ├── api_server.py          # HTTP API、上传和运行接口
│   │   ├── graph.py               # 工作流校验与拓扑排序
│   │   ├── executor.py            # 按画布快照逐节点执行
│   │   ├── registry.py            # 画布节点类型到业务函数的映射
│   │   ├── run_store.py           # 后台任务、状态和人工检查点
│   │   └── workflow_store.py      # SQLite 工作流保存、打开、更新和删除
│   │
│   └── nodes/                     # 各投研节点的实际业务实现
│       ├── start.py
│       ├── project_overview.py
...
Streamlit: app.py
                 ┐
CLI: main.py     ├─→ src/pipeline.py ─→ src/nodes/* ─→ LLM / Search / Files
                 │
拖拽页面          ┘
workflow_web/app.js
  → workflow_server.py
  → visual_workflow/api_server.py
  → visual_workflow/executor.py
  → visual_workflow/registry.py
  → src/pipeline.py / src/nodes/*
```

`registry.py` 是拖拽页面专用的适配层，不包含主要投研逻辑。它把画布节点配置转换成 Pipeline 或节点函数参数；真正的分析 Prompt、检索和报告生成仍在 `src/nodes/` 中。
...
1. **Streamlit 分步向导**：固定 Pipeline，页面分阶段推进。
2. **独立拖拽工作流**：配置画布节点、连线、模型和 Prompt 后一次启动。
3. **CLI**：通过命令行参数运行，适合脚本和批处理。

三者共用同一套 `src/pipeline.py` 和 `src/nodes/` 业务实现。下面先用 Streamlit 固定流程说明标准业务顺序：
...
【Streamlit 人工节点 4】竞品矩阵报告审核
  查看逐家结果和统一报告；可以确认，也可以按具体反馈重新汇总或重新分析

【Streamlit 停靠点】补充尽调材料
...
四个人工节点的性质不同：**项目概况、行业分析是“复核 + 按反馈重新生成”**；**竞品确认是筛选候选竞品**；**竞品报告审核可以保留逐家结果重新汇总，也可以按反馈重新检索并分析全部已选竞品**。
...
| | Streamlit 分步向导 | 独立拖拽工作流 | CLI |
| --- | --- | --- | --- |
| 入口 | `app.py` | `workflow_server.py` | `main.py` |
| 启动方式 | `streamlit run app.py` | `python workflow_server.py` | `python main.py "公司名" ...` |
| 流程结构 | 固定 | 运行前可编辑画布 | 固定，由参数控制 |
| 人工交互 | 页面分阶段操作 | 后台检查点暂停，页面审核后继续 | 终端输入或自动批准参数 |
| 适合场景 | 标准项目和业务使用 | 流程组合与节点配置实验 | 批量任务、脚本和 CI |

第 2 节详细说明 Streamlit，第 3 节详细说明独立拖拽工作流，第 4 节说明 CLI；第 5 节以后介绍节点、关键调用关系和报告输出。
...
`.env` 主要供 **CLI（第 4 节）**和**独立拖拽工作流（第 3 节）**使用。拖拽页面也允许在页面顶部临时填写 API Key，页面填写值会覆盖 `.env`，但不会写回文件。

**Streamlit 分步向导（第 2 节）**要求每个访问者在网页侧边栏填写自己的 API Key，不依赖部署者的 `.env`。因此部署给别人使用时，消耗的是访问者自己的 API 额度。
...
全部通过说明代码本身没问题；如果要验证 API Key 是否真的可用，可以运行第 2 节 Streamlit、第 3 节独立拖拽工作流或第 4 节 CLI。

---

## 2. Streamlit 分步向导（固定 Pipeline）

这是第一套网页入口，启动文件是 `app.py`。它按照预设的投研流程逐步推进，不提供节点拖拽、删除或连线功能。适合直接完成一个标准投研项目，以及部署到 Streamlit Community Cloud 给业务人员使用。
...
这是第二套网页入口，启动文件是 `workflow_server.py`，前端文件位于 `workflow_web/`。它与第 2 节的 Streamlit 页面完全独立，不是 Streamlit 页面里的一个子页面。

### 3.1 与 Streamlit 的区别

| 对比项 | Streamlit 分步向导 | 独立拖拽工作流 |
| --- | --- | --- |
| 启动入口 | `streamlit run app.py` | `python workflow_server.py` |
| 默认地址 | `http://localhost:8501` | `http://127.0.0.1:8765` |
| 流程结构 | 固定 Pipeline | 可拖拽、删除、新增和重新连线 |
| 执行方式 | 页面分阶段触发 | 配置完整画布后一次启动 |
| 节点模型 | 使用系统配置 | 每个画布节点实例可单独选择 |
| 节点 Prompt | 使用代码中的默认 Prompt | 每个画布节点实例可修改 Prompt |
| 文件上传 | 在流程进入对应阶段时上传 | 运行前在各节点配置中上传 |
| 人工节点 | 项目概况、行业、竞品选择和竞品报告审核 | 同样支持，通过后台检查点暂停 |
| 适用场景 | 标准项目、业务人员直接使用、Cloud 部署 | 流程实验、专项尽调组合、模型和 Prompt 调试 |

两套页面共用 `src/pipeline.py` 和 `src/nodes/` 中的投研业务代码，但页面状态、启动进程和交互方式不同。运行其中一个页面不需要同时启动另一个。
...
python workflow_server.py --host 127.0.0.1 --port 8765
```

浏览器打开 `http://127.0.0.1:8765`。页面顶部填写 DashScope 和 Serper API Key；不填写时，后台会继续读取 `.env` 中的配置。页面填写值只用于本次后台运行，不写入工作流 JSON、运行记录或报告。

独立拖拽页面使用本地 SQLite 数据库 `data/workflows.db` 保存工作流，不需要登录账号。页面顶部的工作流库提供：

- **保存**：未保存的画布会新增记录；从数据库打开的旧工作流会更新原记录。
- **另存为**：无论当前工作流是否已经保存，都会创建一条新记录。
- **打开**：加载选中的旧工作流，并把该数据库 ID 设为当前工作流；后续点击“保存”仍更新这一条记录。
- **删除**：删除当前工作流的数据库记录，当前画布仍保留，可以重新保存。
- **新建默认流程**：点击“恢复默认”，重新加载默认画布并解除与旧数据库 ID 的绑定。

数据库保存节点名称、描述、坐标、连线、模型和 Prompt。API Key、运行状态、报告及上传文件路径不会写入数据库；重新打开旧工作流后，需要重新上传本次运行使用的文件。

默认画布是完整流程：

```text
开始 → 项目基本概况 → 行业深度分析 → 竞品发现 → 竞品矩阵分析
     → 深度尽调 → 估值分析 → 综合研判与报告输出
```
...
1. 检查需要保留的节点，删除本次不执行的节点。
2. 从左侧节点栏新增需要的单项尽调节点。
3. 检查节点之间的连线，确保上游输出能到达下游节点。
4. 点击“开始”节点填写公司名称、官网、融资轮次、融资金额、行业、项目描述并上传 BP。
5. 点击尽调节点上传对应材料。
6. 按需要配置每个节点的名称、描述、模型和 Prompt。
7. 点击“校验”，确认工作流不存在环、未知节点或非法 Prompt 变量。
8. 点击“运行工作流”。

运行开始后，画布和节点配置会锁定。后台始终使用点击“运行工作流”时提交的工作流快照；运行中不能通过删除节点改变本次任务。项目概况、行业分析、竞品选择和竞品报告审核仍会暂停等待人工操作。

节点缺少所需上游输出时会标记为 `skipped`。竞品发现和竞品分析是不可缺少的输入，不会用空字符串或空报告代替。模型输出无法解析时，系统会先修复并自动切换稳定模型；全部尝试失败时明确终止依赖链，避免生成缺少竞品依据的估值和最终报告。

估值节点允许在只有部分专项尽调时继续执行，并在估值和最终报告中明确披露未执行的尽调范围。

### 3.4 深度尽调节点的两种配置

**配置 A：保留“深度尽调”聚合节点**

该节点内部顺序执行团队、业务、财务、技术/IP、法律五项尽调，然后生成 `DueDiligenceBundle`。五类文件都在这个节点中上传。
...
画布可以把多个单项节点并列连接到“估值分析”。后台按照稳定拓扑顺序逐个执行节点；估值节点会把已经完成的专项结果组装成尽调汇总。

### 3.5 节点模型和 Prompt

全局默认百炼模型为 `qwen3.7-plus`。每个画布节点实例可以单独选择：

- 千问：`qwen3.7-plus`、`qwen3.6-plus`、`qwen3.6-flash`、`qwen3.5-plus`、`qwen3.5-flash`、`qwen-plus`
- DeepSeek：`deepseek-v4-pro`、`deepseek-v4-flash`
- GLM：`glm-5.2`、`glm-5.1`
- Kimi：`kimi-k2.7-code`、`kimi-k2.6`

第三方模型需要先在阿里云百炼控制台开通对应服务，并使用华北 2（北京）地域的 API Key。

节点配置中的模型、名称、描述和 Prompt 只修改当前画布上的节点实例；以后新增的同类型节点仍使用默认值。竞品矩阵分析包含“单家竞品分析”和“竞品统一汇总”两个独立 LLM 步骤，深度尽调聚合节点包含五个独立 LLM 步骤。

Prompt 下方会逐项显示变量代表的材料、材料来源和写法。例如：

```text
{bp_text} = 开始节点上传的 BP 文件解析全文
{competitor_analysis_context} = 完整竞品矩阵报告
{financial_file_text} = 财务尽调节点上传材料的解析全文
```

在 Prompt 中保留 `{bp_text}`，运行时会自动替换为实际 BP 内容。不能添加系统未定义的变量；如果要在 Prompt 中输出字面花括号，应写成 `{{` 和 `}}`。
...
| 融资金额 | 留空 |
| 所属行业 | 人工智能与大模型 |
| 项目描述 | 月之暗面是一家大模型创业公司，提供 Kimi 智能助手及面向开发者和企业的模型 API 服务。 |
| BP 文件 | 留空 |
| 五类尽调材料 | 首次测试全部留空；报告应标注资料不足 |

为了减少真实 API 成本，竞品发现后建议只选择 1-2 家最相关竞品。
...
1. 点击“恢复默认”。
2. 在“开始”节点填写共用测试输入。
3. 在“竞品矩阵分析”中将单家分析设为 `qwen3.6-flash`，统一汇总保留 `qwen3.7-plus`。
4. 点击“校验”后运行。
5. 依次审核项目概况、行业分析、竞品选择和竞品报告。
6. 预期 8 个默认节点全部 `completed`，最终报告包含五项尽调和估值。

#### 手工测试场景 B：仅团队尽调

1. 恢复默认画布并删除“深度尽调”。
2. 新增“团队尽调”。
3. 连线 `竞品矩阵分析 → 团队尽调 → 估值分析`。
4. 填写共用测试输入并运行。
5. 预期团队尽调、估值和最终报告均为 `completed`。
6. 估值和最终报告应披露未执行业务、财务、技术/IP、法律尽调。

#### 手工测试场景 C：团队 + 财务部分尽调

1. 删除“深度尽调”，新增“团队尽调”和“财务尽调”。
2. 从“竞品矩阵分析”分别连接到两个节点。
3. 从两个节点分别连接到“估值分析”。
4. 填写共用测试输入并运行。
5. 预期估值和最终报告正常完成，并披露只执行了团队与财务尽调。

#### 手工测试场景 D：五个专项尽调分支

1. 删除“深度尽调”，新增五个专项尽调节点。
2. 从“竞品矩阵分析”分别连接到五个专项节点。
3. 从五个专项节点分别连接到“估值分析”。
4. 为各节点选择模型、上传材料并运行。
5. 预期五个专项、估值和最终报告全部 `completed`。
...
项目目录和入口已经放在 README 开头。本节只说明 Pipeline 方法之间的边界。
...
调用方可以决定在哪一步停下来等人工操作。同一套节点逻辑同时支持 CLI 的阻塞式终端交互、Streamlit 跨多次页面请求的阶段状态，以及独立拖拽页面的后台检查点，不需要复制业务实现：
...
- `run_intake_through_discovery()` / `run()` 是便捷封装，默认不传 review callback 时无人值守执行；CLI 和 Streamlit 在这些方法之上连接各自的交互逻辑。
- 独立拖拽页面由 `WorkflowExecutor` 按画布拓扑顺序调用 `registry.py` 中的节点适配函数，再进入同一个 `BPPipeline` 或 `src/nodes/` 实现。
